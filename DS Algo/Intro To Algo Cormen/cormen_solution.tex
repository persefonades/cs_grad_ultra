...
\begin{enumerate}
	...
	\item[A.]		
		\begin{algorithm}[H]
			\Input{A $\longleftarrow$ Array}
			\Output{left, right $\longleftarrow$ starting \& ending indexes of subarray in the array}

			$left \longleftarrow 0$\;
			$right \longleftarrow 0$\;
			$max \longleftarrow A[0]$\;
			\tcc{Increment left end of subarray}
			\For{$i \longleftarrow 0$ \To $n-1$}
			{
				$curSum \longleftarrow 0$\;
				\tcc{Increment right end of subarray}
				\For{$j \longleftarrow i$ \To $n-1$}
				{
...
\begin{enumerate}
	...
	\item[A.]
		\begin{algorithm}[H]
		\tcc{Let $A[i \dots j][k \dots m]$ denote the submatrix of $A$ consisting of rows $i$ through $j$ and columns $k$ through $m$.}
		
		\If{$A$.length = 1}
		{
			\Return $A[0] . B[0]$
		}
		\tcc{Let $C$ be a new $n \times n$ matrix}
